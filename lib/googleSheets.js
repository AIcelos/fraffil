...
class GoogleSheetsService {
  ...
  async getAllData() {
    try {
      ...
      const range = `${sheetName}!A:C`; // Use dynamic sheet name
  ...
  async getInfluencerStats(influencer) {
    try {
      ...
      // For now, we'll estimate revenue per order
      // Later you can add an amount column to your spreadsheet
      const estimatedRevenuePerOrder = 156.27; // Average from mock data
      const totalRevenue = totalSales * estimatedRevenuePerOrder;
      
      // Conversion rate (you'll need click data for real calculation)
      const estimatedConversionRate = 3.2; // Placeholder
      ...
      const recentOrders = sortedData.slice(0, 5).map(order => ({
        date: order.date,
        orderId: order.orderId,
        amount: estimatedRevenuePerOrder // Placeholder
      }));
      ...
      data.forEach(order => {
        ...
        monthlyStats[monthYear].revenue += estimatedRevenuePerOrder;
      });
